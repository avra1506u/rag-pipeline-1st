...
5. A user's question is embedded with the same MiniLM model.
6. The vector store returns the most similar chunks via cosine similarity, filtered by a minimum relevance score.
7. Retrieved chunks are assembled into a prompt alongside the question.
8. **Groq** (`openai/gpt-oss-20b`) generates a grounded answer, returned with source citations, a confidence score, and running conversation history.
